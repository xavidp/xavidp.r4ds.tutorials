...
<!-- PC: Deleted the old gif. If new gif is created, consider deleting old PNGs -->
...
<!-- PC: gif does not match the instructions. has been deleted but a new correct gif should be considered-->
...
Instead of running your code expression-by-expression, you can also execute the complete script in one step with `Cmd/Ctrl + Shift + S`. Doing this regularly is a great way to ensure that you’ve captured all the important parts of your code in the script. But note that this short cut key is not the same thing as Source. In particular, Source causes every command in the script to be echoed.
...
Click `File -> New File` at the top left of your RStudio window, and select "Quarto Document...". You will be offered the option of providing a "Title" and "Author" for the document. **Note that the title of a document has no necessary connection to the name of the file itself.** Add “My First Quarto Document” as the title. Add your name as the author. Click `Create`.

<!-- PC: Consider making new GIF to match instructions. Old GIF deleted -->
...
RStudio automatically saves the contents of the editor when you quit, and automatically reloads it when you re-open. Nevertheless, it’s a good idea to avoid Untitled1, Untitled2, Untitled3, and so on and instead save your documents and give them informative names.
